                ...
                <tr class="hover:bg-[#f8fafc] transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="text-[0.8rem] font-bold text-text-main">${{ number_format($item->price_per_box) }} <small class="text-text-muted font-normal">/caja</small></span>
                            <span class="text-[0.8rem] font-bold text-brand-accent">${{ number_format($item->price_per_unit) }} <small class="text-text-muted font-normal">/unidad</small></span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="flex justify-end gap-2 text-text-muted">
                            <form method="POST" action="{{ route('inventory.destroy', $item->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1 hover:text-red-500 transition-colors">
        ...
        <form method="POST" action="{{ route('inventory.store') }}" class="p-6 flex flex-col gap-4" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4 text-left">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="number" name="price_per_box" required class="px-4 py-2 bg-slate-50 border border-border-subtle rounded-lg text-sm">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[0.65rem] font-bold text-text-muted uppercase">Precio Unidad ($)</label>
                    <input type="number" name="price_per_unit" required class="px-4 py-2 bg-slate-50 border border-border-subtle rounded-lg text-sm">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4 text-left">
                <div class="flex flex-col gap-1.5">
