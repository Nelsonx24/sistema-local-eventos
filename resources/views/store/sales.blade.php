...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-6 py-4 bg-white rounded-lg border border-border-subtle shadow-sm">
        <h3 class="font-semibold text-text-main">Ventas de Tienda</h3>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('store.index') }}" class="bg-slate-800 text-white px-4 py-2 rounded-[6px] text-[0.75rem] font-bold hover:bg-slate-700 flex items-center gap-2 shadow-sm transition-all">
    ...
    <div class="bg-white rounded-xl border border-border-subtle shadow-sm p-12 text-center">
        <div class="flex flex-col items-center gap-3">
            ...
            <a href="{{ route('store.index') }}" class="px-4 py-2 bg-slate-800 text-white rounded-xl text-[0.75rem] font-bold hover:bg-slate-700 transition-all">Ir a Productos</a>
        </div>
    </div>
    @else
            ...
            <tbody class="divide-y divide-[#f1f5f9]">
                ...
                <tr class="hover:bg-[#f8fafc] transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        <span class="text-[0.8rem] text-text-muted font-mono">${{ number_format($sale->unit_price, 2) }}</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-[0.875rem] font-bold text-text-main font-mono">${{ number_format($sale->total_amount, 2) }}</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-[0.8rem] font-bold text-emerald-600 font-mono">+${{ number_format($sale->profit, 2) }}</span>
                    </td>
                </tr>
                @empty
                <tr>
...
<div id="sale-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4" onclick="if(event.target===this)closeModal('sale-modal')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        ...
        <form method="POST" action="{{ route('store.sale.process') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="store_product_id" required
                    ...
                    <option value="{{ $product->id }}" data-stock="{{ $product->stock }}">
                        {{ $product->name }} — Stock: {{ $product->stock }} — ${{ number_format($product->sale_price, 2) }}
                    </option>
                    @endforeach
                </select>
            </div>
