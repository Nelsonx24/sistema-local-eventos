        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-border-subtle shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-border-subtle bg-[#f8fafc] flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-black text-text-main tracking-tight">Tienda</h2>
                        <p class="text-[0.7rem] text-text-muted">{{ $products->total() }} productos registrados</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.outside="open = false" class="absolute right-0 top-full mt-1 bg-white rounded-xl border border-border-subtle shadow-lg py-1 min-w-[200px] z-40">
                            <a href="{{ route('store.products.report') }}" target="_blank" class="block px-4 py-2.5 text-[0.75rem] font-medium text-text-main hover:bg-[#f8fafc] transition-colors">Productos Actual</a>
                            <a href="{{ route('store.products.report.images') }}" target="_blank" class="block px-4 py-2.5 text-[0.75rem] font-medium text-text-main hover:bg-[#f8fafc] transition-colors">Productos con Imágenes</a>
                            <a href="{{ route('store.products.report.sales') }}" target="_blank" class="block px-4 py-2.5 text-[0.75rem] font-medium text-text-main hover:bg-[#f8fafc] transition-colors">Ventas</a>
                        </div>
                    </div>
                    @if(Auth::guard('staff')->user()->role === 'Administrador')
            ...
            <div class="p-12 text-center">
                <div class="flex flex-col items-center gap-3">
                    ...
                    <p class="text-slate-400 font-medium">No hay productos registrados</p>
                    @if(Auth::guard('staff')->user()->role === 'Administrador')
                    <button onclick="openModal('create-modal')" class="px-4 py-2 bg-slate-800 text-white rounded-xl text-[0.75rem] font-bold hover:bg-slate-700 transition-all">Agregar Producto</button>
                    @endif
                </div>
            </div>
            @else
                ...
                <table class="w-full border-collapse text-left">
                    <thead>
                        <tr class="bg-[#f8fafc] border-b border-border-subtle">
                            <th class="px-6 py-3 text-[0.65rem] font-bold text-text-muted uppercase tracking-widest">Producto</th>
                            ...
                            <th class="px-6 py-3 text-[0.65rem] font-bold text-text-muted uppercase tracking-widest">Caduca</th>
                        ...
                        <tr class="hover:bg-[#f8fafc] transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <img src="{{ $product->image_url }}" alt="" class="w-10 h-10 rounded-lg object-cover border border-border-subtle flex-shrink-0">
                                    @endif
                                    <div>
                                        <p class="text-[0.85rem] font-bold text-text-main leading-none mb-0.5">{{ $product->name }}</p>
                                        @if($product->detail)
                                        <p class="text-[0.6rem] text-text-muted truncate max-w-[180px]">{{ $product->detail }}</p>
                                        @endif
                                        @if($product->barcode)
                                        <p class="text-[0.6rem] text-text-muted font-mono">{{ $product->barcode }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-[0.7rem] text-text-muted">{{ $product->category }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-[0.75rem] text-text-muted font-mono font-bold">Bs.{{ rtrim(rtrim(number_format($product->cost, 2), '0'), '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-[0.75rem] font-bold text-brand-accent font-mono">Bs.{{ rtrim(rtrim(number_format($product->sale_price, 2), '0'), '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($product->stock > 5)
                                <span class="inline-flex items-center px-2.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-full font-mono font-bold text-[0.75rem]">{{ $product->stock }}</span>
                                @elseif($product->stock > 0)
                                <span class="inline-flex items-center px-2.5 py-0.5 bg-amber-50 text-amber-700 rounded-full font-mono font-bold text-[0.75rem]">{{ $product->stock }}</span>
                                @else
                            ...
                            <td class="px-6 py-4">
                                ...
                                <span class="text-[0.7rem] text-text-muted">{{ \Carbon\Carbon::parse($product->expiration_date)->format('d/m/y') }}</span>
                                @else
                                <span class="text-[0.7rem] text-slate-300">—</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button onclick="showDetail('{{ $product->id }}', 'products')" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Ver Detalles">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                    </button>
                                    @if(Auth::guard('staff')->user()->role === 'Administrador')
                                    <button onclick="openStockModal('{{ $product->id }}', '{{ $product->name }}', 'products')" class="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Agregar Stock">
                                    ...
                                    <a href="{{ route('store.products.edit', $product) }}" class="p-1.5 text-slate-400 hover:text-brand-accent hover:bg-brand-gold/5 rounded-lg transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('store.products.destroy', $product) }}" onsubmit="return confirm('¿Eliminar {{ $product->name }}?')" class="inline">
                                        @csrf @method('DELETE')
            ...
            <div class="px-6 py-3 border-t border-border-subtle bg-[#f8fafc] flex items-center justify-between text-[0.7rem] text-text-muted">
                <span>{{ $products->where('stock', 0)->count() }} sin stock en esta página</span>
                <div class="flex items-center gap-4">
                    <a href="{{ route('store.products.sales') }}" class="font-bold text-brand-accent hover:text-brand-gold-dark transition-colors flex items-center gap-1.5">
                        Ver Ventas
        ...
        <div class="bg-white rounded-2xl border border-border-subtle shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-border-subtle bg-[#f8fafc] flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[0.7rem] text-text-muted">Últimas ventas de productos</p>
                </div>
            </div>
            @php
            $recentSales = \App\Models\StoreSale::with('product')->latest()->take(5)->get();
            @endphp
            ...
            <div class="divide-y divide-[#f1f5f9]">
                ...
                <div class="px-6 py-3.5 flex items-center justify-between hover:bg-[#f8fafc] transition-colors">
                    <div>
                        <p class="text-[0.8rem] font-bold text-text-main leading-none mb-0.5">{{ $sale->product->name }}</p>
            ...
            @endif
            <a href="{{ route('store.products.sales') }}" class="block px-6 py-3.5 border-t border-border-subtle text-center text-[0.75rem] font-bold text-brand-accent hover:bg-[#f8fafc] transition-colors">
                Registrar Venta
            </a>
        </div>
    </div>
...
<div id="detail-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4" onclick="if(event.target===this)closeModal('detail-modal')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-black text-text-main tracking-tight">Detalles del Producto</h3>
...
<div id="types-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4" onclick="if(event.target===this)closeModal('types-modal')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-black text-text-main tracking-tight">Tipos de Productos</h3>
            <button onclick="closeModal('types-modal')" class="p-2 text-slate-400 hover:text-text-main rounded-lg transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>
            </button>
        </div>

        <form method="POST" action="{{ route('store.products.types.store') }}" class="flex gap-2 mb-5">
            @csrf
        ...
        <div class="space-y-1 max-h-64 overflow-y-auto">
            ...
            <div class="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-[#f8fafc] transition-colors">
                ...
                <form method="POST" action="{{ route('store.products.types.destroy', $type) }}" onsubmit="return confirm('¿Eliminar este tipo?')">
                    @csrf
...
<div id="create-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4 overflow-y-auto" onclick="if(event.target===this)closeModal('create-modal')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-8 p-8">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-black text-text-main tracking-tight">Nuevo Producto</h3>
            <button onclick="closeModal('create-modal')" class="p-2 text-slate-400 hover:text-text-main rounded-lg transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>
            </button>
        </div>

        <form method="POST" action="{{ route('store.products.store') }}" class="space-y-4" enctype="multipart/form-data">
            @csrf
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-[0.7rem] font-bold text-text-muted uppercase tracking-widest mb-1.5">Caducidad</label>
...
function showDetail(id, type) {
    var route = '/store/' + (type === 'products' ? 'productos/' : 'regalos/') + id;
    fetch(route)
        ...
        .then(function(data) {
            ...
            if (data.expiration_date) {
                html += '<div class=\"flex justify-between py-2 border-t border-border-subtle\"><span class=\"font-bold text-text-muted\">Fecha de Caducidad</span><span class=\"text-text-main text-right\">' + new Date(data.expiration_date).toLocaleDateString('es-MX') + '</span></div>';
            }
