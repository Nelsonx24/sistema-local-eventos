...
@section('title', 'Editar Regalo - Tienda')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-border-subtle shadow-sm p-8">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h2 class="text-xl font-black text-text-main tracking-tight">Editar Regalo</h2>
                <p class="text-[0.8rem] text-text-muted">{{ $gift->name }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('store.gifts.update', $gift) }}" class="space-y-5" enctype="multipart/form-data">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $gift->name) }}" required
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="detail" rows="2"
                        class="w-full px-4 py-3 bg-white border border-border-subtle rounded-xl text-[0.875rem] text-text-main placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-brand-gold/40 focus:border-brand-accent transition-all resize-none">{{ old('detail', $gift->detail) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" min="0" name="cost" value="{{ old('cost', $gift->cost) }}" required
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-text-muted font-bold text-[0.875rem]">Bs.</span>
                        <input type="number" step="0.01" min="0" name="sale_price" value="{{ old('sale_price', $gift->sale_price) }}" required
                            class="w-full pl-11 pr-4 py-3 bg-white border border-border-subtle rounded-xl text-[0.875rem] text-text-main placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-brand-gold/40 focus:border-brand-accent transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="category" required
                        ...
                        <option value="{{ $type->name }}" @selected(old('category', $gift->category) == $type->name)>{{ $type->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" min="0" name="stock" value="{{ old('stock', $gift->stock) }}" required
                        class="w-full px-4 py-3 bg-white border border-border-subtle rounded-xl text-[0.875rem] text-text-main placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-brand-gold/40 focus:border-brand-accent transition-all">
                </div>

                <div>
                    <label class="block text-[0.75rem] font-bold text-text-muted uppercase tracking-widest mb-2">Código de Barras</label>
                    <input type="text" name="barcode" value="{{ old('barcode', $gift->barcode) }}" placeholder="Opcional"
                        class="w-full px-4 py-3 bg-white border border-border-subtle rounded-xl text-[0.875rem] text-text-main placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-brand-gold/40 focus:border-brand-accent transition-all">
                </div>
                ...
                <div>
                    ...
                    @if($gift->image_url)
                    <div class="mb-2">
                        <img src="{{ $gift->image_url }}" alt="{{ $gift->name }}" class="w-20 h-20 object-cover rounded-lg border border-border-subtle">
                    </div>
                    @endif
