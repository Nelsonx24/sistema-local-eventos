...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-6 py-4 bg-white rounded-lg border border-border-subtle shadow-sm">
        <h3 class="font-semibold text-text-main">Ventas de Regalos</h3>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('store.gifts.sales', ['filter' => 'today']) }}" class="{{ request('filter') === 'today' ? 'bg-brand-gold text-white' : 'bg-white text-slate-700 border border-border-subtle' }} px-4 py-2 rounded-[6px] text-[0.75rem] font-bold hover:bg-brand-gold/90 hover:text-white flex items-center gap-2 shadow-sm transition-all">
                Hoy
            </a>
            <a href="{{ route('store.gifts.sales') }}" class="{{ !request('filter') ? 'bg-brand-gold text-white' : 'bg-white text-slate-700 border border-border-subtle' }} px-4 py-2 rounded-[6px] text-[0.75rem] font-bold hover:bg-brand-gold/90 hover:text-white flex items-center gap-2 shadow-sm transition-all">
                Todos
            </a>
            <a href="{{ route('store.gifts') }}" class="bg-slate-800 text-white px-4 py-2 rounded-[6px] text-[0.75rem] font-bold hover:bg-slate-700 flex items-center gap-2 shadow-sm transition-all">
    ...
    <div class="bg-white rounded-xl border border-border-subtle shadow-sm p-12 text-center">
        <div class="flex flex-col items-center gap-3">
            ...
            <p class="text-slate-400 font-medium">No hay regalos con stock disponible</p>
            <a href="{{ route('store.gifts') }}" class="px-4 py-2 bg-slate-800 text-white rounded-xl text-[0.75rem] font-bold hover:bg-slate-700 transition-all">Ir a Regalos</a>
        </div>
    </div>
    @else
        ...
        <table class="w-full border-collapse text-left">
            <thead>
                <tr class="bg-[#f8fafc] border-b border-border-subtle">
                    ...
                    <th class="px-6 py-3 text-[0.7rem] font-bold text-text-muted uppercase tracking-widest">Regalo</th>
            ...
            <tbody class="divide-y divide-[#f1f5f9]">
                ...
                <tr class="hover:bg-[#f8fafc] transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-[0.875rem] font-bold text-text-main">{{ $sale->gift->name }}</span>
                    </td>
...
<div id="sale-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4" onclick="if(event.target===this)closeModal('sale-modal')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        ...
        <form method="POST" action="{{ route('store.gifts.sale.process') }}" class="space-y-5">
            @csrf

            <div>
                <label class="block text-[0.75rem] font-bold text-text-muted uppercase tracking-widest mb-2">Regalo</label>
                <select name="store_gift_id" required
