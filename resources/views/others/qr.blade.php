...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div class="flex items-center gap-4 mb-8">
            <div class="w-12 h-12 bg-brand-gold/10 text-brand-gold rounded-2xl flex items-center justify-center shadow-sm">
        ...
        <form method="POST" action="{{ route('others.qr.update') }}" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            @csrf
            <div class="space-y-6">
                <div class="p-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-4 group hover:border-brand-accent transition-all">
                    ...
                    <label for="qr-input" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest cursor-pointer hover:text-brand-accent">Cambiar Imagen</label>
                </div>
            </div>

            <div class="space-y-6">
                <div class="bg-brand-gold/10 border border-brand-gold/20 p-4 rounded-xl flex gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-brand-gold shrink-0 mt-0.5"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>
                ...
                <button type="submit" class="w-full py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg bg-black text-white shadow-black/20 hover:bg-slate-900">
